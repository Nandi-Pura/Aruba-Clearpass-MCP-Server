...
def _normalise_mac(mac: str) -> str:
    ...
    if len(clean) != 12:
        raise ValueError(
            f"Invalid MAC address '{mac}'. Expected 12 hex digits (with or without separators)."
        )
...
def register(mcp: FastMCP, settings: Settings) -> None:  # noqa: ARG001
    ...
    async def find_endpoint_by_mac(mac_address: str) -> dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                return format_error(
                    f"Endpoint with MAC {normalised} not found in ClearPass.",
                    status_code=404,
                    path=path,
                )
            return format_error(
    ...
    async def get_endpoint_insight(mac_or_ip: str) -> dict[str, Any]:
        ...
        is_mac = bool(
            re.match(r"^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$", mac_or_ip) or 
            re.match(r"^[0-9A-Fa-f]{12}$", mac_or_ip)
        )
        ...
        try:
            ...
            if e.response.status_code == 404:
                return format_error(
                    f"No Insight record found for '{mac_or_ip}'.",
                    status_code=404,
                    path=path,
                )
            return format_error(
