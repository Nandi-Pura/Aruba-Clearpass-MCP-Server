...
async def _paginate_items(
    ...
    """
    Aggregate paginated list results from *path* into a single response dict.

    Returns ``{"items": [...], "total": int, "retrieved": int, "truncated": bool}``,
    or an error dict on HTTP / connection failure.
    """
    items: list[Any] = []
    total = 0
    truncated = False
    ...
    try:
        async for page in get_client().paginate(path, params=params, max_pages=max_pages):
...
async def search_audit_records(
    ...
    return await _paginate_items("/audit-record", params, max_pages)
