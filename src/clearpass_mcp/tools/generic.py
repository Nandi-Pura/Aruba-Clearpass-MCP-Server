...
def _get_settings() -> Settings:
    if _settings is None:  # pragma: no cover
        raise RuntimeError("generic tools not registered yet")
    return _settings
...
def _dry_run_response(
    *,
    ...
    """Log a dry-run audit entry and return a dry-run preview dict."""
...
async def _run_write(
    *,
    ...
    """
    Execute one write request, emit an audit entry, and return the result.

    Handles ``httpx.HTTPStatusError`` and generic exceptions so each tool
    body stays a single ``await _run_write(...)`` call.
    """
...
async def clearpass_post(
    ...
    if dry_run:
        return _dry_run_response(tool="clearpass_post", path=path, method="POST", body=body)
    return await _run_write(tool="clearpass_post", path=path, method="POST", body=body)
...
async def clearpass_patch(
    ...
    if _get_settings().CLEARPASS_READ_ONLY:
        return _read_only_error(path)
    if dry_run:
        return _dry_run_response(tool="clearpass_patch", path=path, method="PATCH", body=body)
    return await _run_write(tool="clearpass_patch", path=path, method="PATCH", body=body)
...
async def clearpass_put(
    ...
    if _get_settings().CLEARPASS_READ_ONLY:
        return _read_only_error(path)
    if dry_run:
        return _dry_run_response(tool="clearpass_put", path=path, method="PUT", body=body)
    return await _run_write(tool="clearpass_put", path=path, method="PUT", body=body)
...
async def clearpass_delete(
    ...
    if _get_settings().CLEARPASS_READ_ONLY:
        return _read_only_error(path)
    if dry_run:
        return _dry_run_response(tool="clearpass_delete", path=path, method="DELETE", body=None)
    return await _run_write(tool="clearpass_delete", path=path, method="DELETE", body=None)
...
def register(mcp: FastMCP, settings: Settings) -> None:
    ...
    mcp.tool()(clearpass_get)
    mcp.tool()(clearpass_list_apis)
    mcp.tool()(clearpass_post)
    mcp.tool()(clearpass_patch)
    mcp.tool()(clearpass_put)
    mcp.tool()(clearpass_delete)
